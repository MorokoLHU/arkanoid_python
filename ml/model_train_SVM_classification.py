...
x_train, x_val, y_train, y_val = train_test_split(x_train, y_train, test_size=0.7)
...
model = SVC(C = C_value, kernel = ker,verbose = False)
...
with open(os.path.join(os.path.dirname(__file__),'save',\
    "SVM_CL_ker={}_C={}.pickle" .format(ker,C_value)),'wb') as f:
    pickle.dump(model,f)
