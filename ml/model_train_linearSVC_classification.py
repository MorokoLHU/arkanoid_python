...
# 線性SVC模型
php_C = float(sys.argv[1])  # 接收PHP參數
best_model = None

best_accuracy = 0

print(f"start C= {php_C} \n")
model = LinearSVC(C=php_C)
model.fit(x_train, y_train)
y_pred_val = model.predict(x_val)
acc = accuracy_score(y_val, y_pred_val)
print(f"C = {php_C}, Accuracy = {acc:.3f}")
...
print(f"C = {php_C}, Test Accuracy = {test_accuracy:.3f}, Test F1 = {test_f1:.3f}")
...
model_filename = f"LinearSVC_CL_C={php_C}_acc={test_accuracy:.2f}.pickle"
